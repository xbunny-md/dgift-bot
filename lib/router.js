...
export async function handleMessages(sock, m, botSettings) {
  try {
    ...
    // Owner detection now uses sock.user.id only. Supabase owner_number is ignored.
    ...
    sock.sendMessage = async (jid, content, options = {}) => {
      ...
      try {
        ...
        if (typeof content.text === 'string' &&!content.image &&!content.video &&!content.audio &&!content.document) {
          content.text = await processText(content.text)
        }

        if (typeof content.caption === 'string' && (content.image || content.video || content.document)) {
          content.caption = await processText(content.caption)
        }

      } catch (e) {
        console.log('Design wrapper failed:', e.message)
      }
