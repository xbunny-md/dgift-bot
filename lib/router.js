...
function resolveLid(participants = [], jid) {
  if (!jid) return jid
...
function extractMessage(msg) {
  ...
  return {
    ...
    msgType,
    isViewOnce,
    reaction: message.reactionMessage || null
  }
}
    ...
    if (stat.isDirectory()) {
      ...
      try {
        ...
        if (commands.has(cmdName)) {
          ...
          console.log(
            `[WARN] Command ${cmdName} from ${cmdData.category} overwrote ${existingCmd.category}`
          )
        }
...
export async function handleMessages(sock, m, botSettings) {
  try {
    ...
    if (contextInfo) {
      if (contextInfo.mentionedJid?.length > 0) {
        contextInfo.mentionedJid =
          contextInfo.mentionedJid.map(jid =>
            normalizeJid(resolveLid(participants, jid))
          )
      }
      ...
      if (contextInfo.participant) {
        contextInfo.participant =
          normalizeJid(
            resolveLid(participants, contextInfo.participant)
          )
      }
    }
    ...
    const ownerVariants = getOwnerVariants(botJid)
    ...
    /*
      IMPORTANT FIX:
      msg.key.fromMe is the ONLY safe way
      to detect real bot messages.
    */
    ...
    const stableMode =
      botSettings?.stable_mode !== false

    const antiBanMode =
      botSettings?.anti_ban_mode === true
    ...
    const args = body
      .slice(botSettings.prefix.length)
      .trim()
      .split(/ +/)
    ...
    if (!command) {
      if (matchArray.length > 1) {
        let listText =
          `"${providedName}" matches ${matchArray.length} commands:\n\n`

        matchArray.forEach((cmdName, index) => {
          const cmd = commands.get(cmdName)

          listText +=
            `${index + 1}. ${botSettings.prefix}${cmdName} - ` +
            `${cmd.desc} [${cmd.category}]\n`
        })
    ...
    console.log(
      `[CMD] ${command.name} from ${pushName} [${sender}]`
    )
