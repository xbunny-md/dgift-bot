  ...
  for (const item of items) {
    ...
    if (stat.isDirectory()) {
      ...
      try {
        ...
        // Allow overwrite, log if duplicate
...
export async function handleMessages(sock, m, botSettings) {
  try {
    ...
    // Extract body, type, viewonce, reaction
    const { body, msgType, isViewOnce, reaction } = extractMessage(msg)
    
    // Prevent infinite loop
    if (msg.key.fromMe && body.startsWith(botSettings.prefix)) return
    ...
    // If no body and not reaction, skip command check
