...
export async function initializeRouter(botSettings) {
  ...
  isLoaded = true
  const brand = botSettings?.brand_name
...
export async function handleMessages(sock, m, botSettings) {
  try {
    ...
    const stableMode = botSettings?.stable_mode
    const antiBanMode = botSettings?.anti_ban_mode
    ...
    const args = body.slice(botSettings?.prefix?.length || 0).trim().split(/ +/)
