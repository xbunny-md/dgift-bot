...
export async function initializeRouter(botSettings) {
  ...
  isLoaded = true
  const brand = botSettings?.brand_name // FIX: Ongeza?.
...
export async function handleMessages(sock, m, botSettings) {
  try {
    ...
    const pushName = msg?.pushName // FIX: Ongeza?.
    ...
    const vipNumbers = botSettings?.vip_numbers // FIX: Ongeza?.
    const isVIP = vipNumbers?.includes(senderNumber) // FIX: Ongeza?.
    ...
    const restrictedCategories = botSettings?.restricted_categories // FIX: Ongeza?.
    const stableMode = botSettings?.stable_mode // FIX: Ongeza?.
    const antiBanMode = botSettings?.anti_ban_mode // FIX: Ongeza?.
    ...
    // FIX 2: Observers sasa wanapata wrappedSettings pia
    ...
    if (!body.startsWith(botSettings?.prefix)) return // FIX: Ongeza?.

    const args = body.slice(botSettings?.prefix?.length || 0).trim().split(/ +/) // FIX: Ongeza?.
    ...
    const ownerMode = botSettings?.owner_mode // FIX: Ongeza?.
    const publicMode = botSettings?.public_mode // FIX: Ongeza?.
    const privatePublicMode = botSettings?.private_public_mode // FIX: Ongeza?.
    const isRestricted = command.restricted === true || restrictedCategories?.includes(command.category) // FIX: Ongeza?.
